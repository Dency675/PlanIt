...
const updateUserRoleToProjectManager=  async (req: Request, res: Response) => {
  ...
  try {
    const projectManagerRole = await Roles.findOne({ where: { role_name: 'project manager' } });

    if (!projectManagerRole) {
      return res.status(404).json({ error: 'Project manager role not found' });
    }

    let employeeRoleMapping = await EmployeeRoleMapping.findOne({ where: { user_id } });

    if (!employeeRoleMapping) {
      return res.status(404).json({ error: 'Employee role mapping not found for the user' });
    }
